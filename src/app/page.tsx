...
export default function Home() {
  ...
  // Input auto-width functionality
  const inputRef = useRef<HTMLInputElement>(null);
  const [inputWidth, setInputWidth] = useState("30px"); // Start with small width
  ...
  useEffect(() => {
    if (inputRef.current) {
      ...
      // Set minimum width and add generous padding
      const minWidth = 30;
      const padding = 16; // more padding to ensure all text is visible
  ...
  const currentQuestion =
    currentQuestionIndexInOrder !== undefined
      ? vocabularyData[currentQuestionIndexInOrder]
      : null;
  ...
  const shouldShowComplete =
    questionOrder.length === 1 && showFeedback && isCorrectAnswer;
  ...
  const checkAnswer = async (origin: string, sentence: string) => {
    try {
      ...
      const response = await fetch(
        "https://whlfxkvrmdzgscnlklmn.supabase.co/functions/v1/hanvoca",
        {
          method: "POST",
          headers: {
            Authorization:
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 영어 문장: 정답 단어에 [ ] 표시
    const origin =
      currentQuestion?.english_translation.replace(
    ...
    const sentence =
      currentQuestion?.korean_sentence.replace(/____/, `[${userWord}]`) || "";

    try {
      const { feedback: feedbackMessage, accuracy } = await checkAnswer(
        origin,
        sentence
      );
  ...
  const goToNextQuestion = () => {
    ...
    if (isCorrectAnswer) {
      const currentQuestionId = questionOrder[currentQuestionIndex];
      const newQuestionOrder = questionOrder.filter(
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-100 to-purple-100 p-4">
      <div className="max-w-md mx-auto pt-12">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">Hanvoca</h1>
          <p className="text-gray-600 text-sx">
            Explore Korean vocabulary daily together!
          </p>
          <p className="text-gray-600 text-sm">
            A new set of words is available every day.
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
            ...
            <div className="bg-blue-50 rounded-lg p-4">
              ...
              <div className="flex items-center justify-center">
                <div className="bg-white rounded-full px-3 py-1 text-sm text-gray-600 border border-yellow-300">
                  <span className="text-yellow-600">✨</span> Translate the
                  highlighted word to Korean
                </div>
              </div>
            </div>
          </div>
            ...
            <p className="text-lg text-gray-800 leading-relaxed">
              ...
              <span className="relative inline-block">
                <input
                  ...
                  className="px-2 py-1 border-b-2 border-blue-400 bg-blue-50 text-center font-medium focus:outline-none focus:border-blue-600 focus:bg-blue-100"
                  placeholder="?"
              ...
              {showHints && (
                <div className="space-y-3 bg-orange-50 p-4 rounded-lg">
                  ...
                  <div>
                    <button
                      onClick={() => {
                        if (!hintsUsed.includes("semantic"))
                          setHintsUsed([...hintsUsed, "semantic"]);
                      }}
                  ...
                  {hintsUsed.includes("semantic") && (
                    <div>
                      <button
                        onClick={() => {
                          if (!hintsUsed.includes("structural"))
                            setHintsUsed([...hintsUsed, "structural"]);
                        }}
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span className="font-medium">
                            🧩 Hint 2: What does it look like?
                          </span>
                        </div>
                      </button>
                      {hintsUsed.includes("structural") && (
                        <div className="mt-2 p-3 bg-green-50 rounded-lg text-green-700 text-sm">
                          {currentQuestion?.hints?.structural?.replace(
                            /\*\*(.*?)\*\*/g,
                            "$1"
                          ) || ""}
                        </div>
                  ...
                  {hintsUsed.includes("structural") && (
                    <div>
                      <button
                        onClick={() => {
                          if (!hintsUsed.includes("discovery"))
                            setHintsUsed([...hintsUsed, "discovery"]);
                        }}
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span className="font-medium">
                            🎁 Hint 3: Show me an answer!
                          </span>
                        </div>
                      </button>
                      {hintsUsed.includes("discovery") && (
                        <div className="mt-2 p-3 bg-green-50 rounded-lg text-green-700 text-sm">
                          {currentQuestion?.hints?.discovery?.replace(
                            /\*\*(.*?)\*\*/g,
                            "$1"
                          ) || ""}
                        </div>
          ...
          {showFeedback && (
            <div className={`p-4 rounded-lg border-2 ${feedbackBoxClass}`}>
              <p className={`font-medium ${feedbackTextClass}`}>{feedback}</p>
            ...
            <div className="bg-white rounded-2xl p-6 max-w-sm w-full max-h-[80vh] overflow-y-auto">
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  {currentQuestion?.acceptable_answers?.best?.map(
                    (word, index) => (
                      <div key={index} className="bg-green-50 p-2 rounded mb-2">
                        ...
                        <p className="text-xs text-green-600 mt-1">
                          {currentQuestion?.learning_notes?.[
                            word as keyof typeof currentQuestion.learning_notes
                          ] || ""}
                        </p>
                      </div>
                    )
                ...
                <div>
                  ...
                  {currentQuestion?.acceptable_answers?.good?.map(
                    (word, index) => (
                ...
                <div>
                  ...
                  {currentQuestion?.acceptable_answers?.acceptable?.map(
                    (word, index) => (
                      <div
                        key={index}
                        className="bg-purple-50 p-2 rounded mb-2"
                      >
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 mb-6">
          ...
          <div className="text-gray-700 text-sm">
            <p>
              What Korean word best expresses '
